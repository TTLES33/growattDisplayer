...
async function loadAllTemperatures() {
    ...
    document.getElementById("shortTempsData").innerHTML = "";
    ...
    for (i = 0; i < tempSensors.length; i++) {
        ...
        let teplota = tempSensors[i].teplota;
        ...
        createSimpleTempBoard(sensorName, teplota);
        createTempBoard(sensorId, sensorName, teplota);
    }
}

//creates board for one sensor with name, value and graph
function createTempBoard(sensorId, sensorName, teplota) {
    ...
    gridName.onclick = async function () {
        await changeSensorName(sensorId, sensorName);
        await loadAllTemperatures();
    }

    gridName.innerHTML = sensorName;

    let gridTemp = document.createElement("div");
    gridTemp.className = "grid_temp";

    let gridTempValue = document.createElement("div");
    gridTempValue.innerHTML = Number(teplota).toFixed(1);
    ...
    createGraph(gridGraph, sensorId);
}

function createSimpleTempBoard(sensorName, teplota){
    const sensorsContainer = document.getElementById("shortTempsData");
    let sensorContainer = document.createElement("div");
        sensorContainer.className = "simpleSensorContainer";
    let sensorNameElement = document.createElement("div");
        sensorNameElement.className = "sensorName";
        sensorNameElement.innerHTML = sensorName;
    let sensorValue = document.createElement("div");
        sensorValue.className = "sensorValue";
        sensorValue.innerHTML = Number(teplota).toFixed(1) + "°C";

    sensorContainer.appendChild(sensorNameElement);
    sensorContainer.appendChild(sensorValue);
